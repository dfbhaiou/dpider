 ...
 *
 */
public class IndexSlaveScheduler implements Scheduler {

	 private static Logger logger = Logger.getLogger(IndexSlaveScheduler.class);
...
//        此处考虑应用dubbo的哪种集群容错策略
        urlSchedulerService.push(simpleRequest,ConstantValue.REDIS_KEY_KW_INDEX);
        return;
    }

    @Override
    public Request poll(Task task) {
        SimpleRequest simpleRequest = urlSchedulerService.poll(ConstantValue.REDIS_KEY_KW_INDEX);
        Request request = new Request();
        if (simpleRequest == null) {
            return request;
        }
        System.out.println("IndexSlaveScheduler poll:" + simpleRequest.getUrl());
